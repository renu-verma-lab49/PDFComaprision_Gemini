...
def set_prompt(title, text):
    prompt = '''
Compate both the file and respond 'Both file are same' if content is same and 'Both the file is different' if content is different
{} '''.format(title, text)
    return prompt
...
def get_testplan_chunks(testplan_docx):
    ...
    token_limit = 4000  # Adjust based on the model's limit
